        ...
        <div className="mt-[170px] grid grid-cols-1 gap-x-[66px] md:grid-cols-[394px_1fr]">
          ...
          <div className="grid grid-cols-1 gap-[18px] md:grid-cols-2">
            {destinations.map((item) => (
              <article
                key={item.title}
                className="group relative h-[330px] overflow-hidden border border-[#e5e1da] bg-white p-[18px] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:border-[#d4cec4] hover:shadow-[0_16px_48px_rgba(0,0,0,0.08)]"
              >
                <div className="absolute inset-0 z-0 overflow-hidden">
                  <div className="absolute bottom-[18px] right-[18px] h-[160px] w-[214px] overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:bottom-0 group-hover:right-0 group-hover:h-full group-hover:w-full">
                    <Image
                      src={item.image}
                      alt={item.title}
                      fill
                      sizes="(max-width: 768px) 100vw, 50vw"
                      className="object-contain object-bottom-right transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:object-cover group-hover:scale-[1.02]"
                    />
                  </div>
                ...
                <div className="relative z-10 flex h-full flex-col justify-between">
                  <h3 className="max-w-[260px] text-[24px] font-medium leading-[1] tracking-[-0.03em] text-[#1f1f1a] transition-colors duration-500 group-hover:text-white">
                    {item.title}
                  </h3>
